        ...
        const data = request.body;
          ...
          if(cid!==cid_re )
            ...
            const ret_insert = await add_fed_data_json(data,cid, author_did, publisher_did, publisher_epoch, author_sig, publisher_sig )
              ...
              reply.status(200).send({cid:cid_re, sig:confirmation}) 
              ...
              if(  e.toString().includes(" duplicate key" ) )
               reply.status(403).send({"cid":cid_re, "error":"Duplicate key insert"});
              else
                reply.status(500).send({"cid":cid_re, "error":e});
            }
          ...
          reply.status(200).send("cid_re:"+cid_re);
       }
